...
char &String::operator[](const int index) {
	if (index < 0 || index >= size) {
		throw std::out_of_range("Ёлемента с данным индексом не существует.");
	}
	else {
		return array[index];
	}
...
String &String::operator--() {
	if (size>=2)
	{
		...
		throw std::length_error("—лишком коротка€ строка дл€ выполнени€ декремента.");
	}
}
...
char *String::AllocatorString::allocate(int size) const {
	if (size < 0) {
		throw std::length_error("–азмер строки не может быть отрицательным числом.");
	}
	if (size == 0) {
		throw std::length_error("–азмер строки не может быть равен нулю.");
	}
	char *temp = new(std::nothrow) char[size] {0};
	if (!temp) {
		throw std::bad_alloc();
	}
	return temp;
...
BitString::BitString(const char *NewStr) : String() {
	if (isBitString(NewStr)) {
		...
		throw std::invalid_argument("Ѕитова€ строка должна содержать '0' или '1'.");
	}
}
...
BitString &BitString::operator<<(signed int num) {
	if (num < 0) {
		throw std::domain_error(" оличество сдвигов должно быть положительным числом.");
	}
	else {
...
BitString operator | (BitString &one, BitString &two) {
	if (one.getSize() != two.getSize()) {
		throw std::domain_error("—троки должны быть одного размера.");
	}
	else {
